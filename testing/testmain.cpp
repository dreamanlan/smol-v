...
#include "external/hash/xxhash.h"
...
static bool ReadFile(const char* fileName, ByteArray& output)
{
...
namespace vk
{
    ...
    enum ShaderProgramFlagBits
    {
        kFragmentShaderHasDiscardBit = (1 << 0),
        kFragmentShaderModifiesDepthBit = (1 << 1),
        kShaderUsesClipDistance = (1 << 2),
        kFragmentShaderHasUAVs = (1 << 3),
        kVK_EXT_shader_viewport_index_layer = (1 << 4),
        kHasShaderStageFlagsBit = (1 << 5),
        kHasCombinedTextureAndSampler = (1 << 6),
        kVersionNumberBits = (0xFF << 24), // bits 24-31 are used for a version number
    };

    typedef uint32_t ShaderProgramFlags;

    enum ShaderStageFlagBits
    {
        kShaderStageUsesBitfieldUExtract = (1 << 0),
        kShaderStageUsesExtNonSemanticInfo = (1 << 1)
    };
    ...
    // Version 1: Added 'attachmentsMask'
    // Version 2: Added 'shaderProgramHashes'
    static const uint8_t kShaderSetHeaderVersion = 2;
    ...
    public:
        ...
        ShaderStageFlags stageFlags[kShaderStageCount];     // only valid if ShaderSetHeader::flags has kHasShaderStageFlagsBit (for compatibility)
        uint32_t attachmentsMask; // only valid for version >= 1
        uint8_t shaderProgramHashes[kShaderStageCount][kHashSizeBytes]; // 1 hash per shader stage. Only valid for version >= 2

    public:
        ShaderProgramFlags GetFlags() const { return flags & kFlagsMask; }
        void SetFlags(ShaderProgramFlags newFlags) { flags = (flags & kVersionNumberBits) | (newFlags & kFlagsMask); }
        void OrFlags(ShaderProgramFlags newFlags) { flags |= newFlags & kFlagsMask; }
        static inline uint8_t GetVersion(ShaderProgramFlags flags) { return flags >> 24; }      // bits 24..31
        uint8_t GetVersion() const { return GetVersion(flags); }      // bits 24..31
        void SetVersion(uint8_t version) { flags = (flags & kFlagsMask) | ((uint32_t)version << 24); }

        const ShaderFormatHeader* GetShaderFormatHeader(ShaderStage shaderType) const
        {
            if (shaderType >= kShaderStageCount)
                return nullptr;

            return &shaders[shaderType];
        }
        ShaderStageFlags GetStageFlags(ShaderStage shaderType) const
        {
            return (flags & kHasShaderStageFlagsBit) && shaderType < kShaderStageCount ? stageFlags[shaderType] : 0;
        }
        bool GetAttachmentMask(uint32_t* attachmentMask) const
        {
            if (GetVersion() < 1)
                return false;

            *attachmentMask = attachmentsMask;
            return true;
        }
        void SetAttachmentMask(uint32_t mask)
        {
            attachmentsMask = mask;
        }
        void SetShaderStage(int index, const ShaderFormatHeader& header, ShaderStageFlags flags, const uint8_t shaderHash[kHashSizeBytes])
        {
            shaders[index] = header;
            stageFlags[index] = flags;
            memcpy(shaderProgramHashes[index], shaderHash, kHashSizeBytes);
        }

        bool CheckValid(size_t shaderBinarySize) const
        {
            if (shaderBinarySize < sizeof(ShaderSetHeader))
                return false;
            for (int i = 0; i < vk::kShaderStageCount; ++i) {
                const ShaderFormatHeader& shaderHeader = shaders[i];
                if (shaderHeader.offsetBytes == 0 || shaderHeader.size == 0)
                    continue;
                if (shaderHeader.offsetBytes < sizeof(ShaderSetHeader) || shaderHeader.offsetBytes + shaderHeader.size > shaderBinarySize)
                    return false;
            }
            return true;
        }
        static inline bool CheckValid(const void* pData, size_t shaderBinarySize)
        {
            auto* pHeader = reinterpret_cast<const ShaderSetHeader*>(pData);
            return pHeader->CheckValid(shaderBinarySize);
        }

        void PrintInfo(int index) const
        {
            if (index >= 0 && index < kShaderStageCount) {
                printf("shader offset: 0x%8.8X\n", shaders[index].offsetBytes);
                printf("shader size: 0x%8.8X\n", shaders[index].size);
                printf("hash for %d:", index);
                for (int i = 0; i < kHashSizeBytes; ++i) {
                    printf(" %2.2X", shaderProgramHashes[index][i]);
                }
                printf("\n");
            }
        }

        bool DecompressShader(size_t shaderBinarySize, const ShaderSetHeader* shaderSetHeader, ShaderStage shaderType, SpirvData& buffer) const
        {
            const uint8_t* shaderSetBase = reinterpret_cast<const uint8_t*>(shaderSetHeader);
            const ShaderFormatHeader* shaderHeader = shaderSetHeader->GetShaderFormatHeader(shaderType);

            if (!shaderHeader || shaderHeader->offsetBytes == 0 || shaderHeader->size == 0)
                return false;

            // Validate the offset relative to the entire shader binary size + smol-v header
            if (shaderHeader->offsetBytes < sizeof(shaderSetHeader) || shaderHeader->offsetBytes + shaderHeader->size > shaderBinarySize)
                return false;

            const uint32_t* dataPtr = reinterpret_cast<const uint32_t*>(shaderSetBase + shaderHeader->offsetBytes);
            uint32_t size = (uint32_t)smolv::GetDecodedBufferSize(dataPtr, shaderHeader->size);

            if (size > 0)
            {
                buffer.resize(size / 4);
                // smol-v version "zero" in Unity was based on code as of 2016-08-31, so decode as that
                bool success = smolv::Decode(dataPtr, shaderHeader->size, buffer.data(), size, smolv::kDecodeFlagUse20160831AsZeroVersion);
                return success;
            }
            else
            {
    ...
    bool SpirvDebugNameFilter(const char* name)
    {
        if (!name)
            return false;
...
int main(int argc, const char* argv[])
{
    ...
    if (error) {
        printf("[usage]:\n");
        printf("\tsmol-v-test -u2s shader.dat -vs vert.spv -fs frag.spv\n");
    ...
    if (isUnity2Spirv) {
        ...
        if (vk::ReadSavedShaderData(dataFile.c_str(), datArray) && vk::ShaderSetHeader::CheckValid(datArray.data(), datArray.size())) {
            ...
            vk::SpirvData vsBuf;
            vk::SpirvData fsBuf;
            pHeader->DecompressShader(datArray.size(), pHeader, vk::kShaderStageVertex, vsBuf);
            pHeader->DecompressShader(datArray.size(), pHeader, vk::kShaderStageFragment, fsBuf);

            if (vsBuf.size() > 0) {
                FILE* fout = fopen(vsFile.c_str(), "wb");
                if (fout) {
                    fwrite(vsBuf.data(), sizeof(uint32_t), vsBuf.size(), fout);
                    fclose(fout);
                }
                else {
            ...
            if (fsBuf.size() > 0) {
                FILE* fout = fopen(fsFile.c_str(), "wb");
                if (fout) {
                    fwrite(fsBuf.data(), sizeof(uint32_t), fsBuf.size(), fout);
                    fclose(fout);
                }
                else {
                    printf("write %s failed.\n", fsFile.c_str());
        ...
        if (vk::ReadSavedShaderData(dataFile.c_str(), datArray) && vk::ShaderSetHeader::CheckValid(datArray.data(), datArray.size())) {
...
#endif
            const int c_hash_size = vk::ShaderSetHeader::kHashSizeBytes;
            uint8_t vsHash[c_hash_size] = { 0 };
            uint8_t fsHash[c_hash_size] = { 0 };
            ByteArray vsArray{};
            ByteArray fsArray{};
            if (!vsFile.empty()) {
                ByteArray spvArray{};
                if (ReadFile(vsFile.c_str(), spvArray)) {
                    XXH128_hash_t r = XXH3_128bits(spvArray.data(), spvArray.size());
                    memcpy(vsHash, &r, c_hash_size);
                    if (!smolv::Encode(spvArray.data(), spvArray.size(), vsArray, flags, vk::SpirvDebugNameFilter))
                    {
            ...
            if (!fsFile.empty()) {
                ByteArray spvArray{};
                if (ReadFile(fsFile.c_str(), spvArray)) {
                    XXH128_hash_t r = XXH3_128bits(spvArray.data(), spvArray.size());
                    memcpy(fsHash, &r, c_hash_size);
                    if (!smolv::Encode(spvArray.data(), spvArray.size(), fsArray, flags, vk::SpirvDebugNameFilter))
                    {
                        printf("encode %s failed.\n", fsFile.c_str());
                        fsArray.clear();
                    }
            ...
            size_t newSize = sizeof(vk::ShaderSetHeader);
            auto* pHeader = reinterpret_cast<vk::ShaderSetHeader*>(datArray.data());
            for (int i = 0; i < vk::kShaderStageCount; ++i) {
                pHeader->PrintInfo(i);

                if (i == vk::kShaderStageVertex && vsArray.size() > 0) {
                    newSize += vsArray.size();
                }
                else if (i == vk::kShaderStageFragment && fsArray.size() > 0) {
                    newSize += fsArray.size();
                }
                else {
                    newSize += pHeader->shaders[i].size;
                }
            }

            //Only for Vulkan all compiled shader stages have to be written out in the same binary blob in this particular order
            const vk::ShaderStage vulkanGraphicsStages[] = { vk::kShaderStageFragment, vk::kShaderStageVertex, vk::kShaderStageHull, vk::kShaderStageDomain, vk::kShaderStageGeometry, vk::kShaderStageSurface };
            ByteArray outArray(newSize);
            memcpy(outArray.data(), pHeader, sizeof(vk::ShaderSetHeader));
            auto* pOutHeader = reinterpret_cast<vk::ShaderSetHeader*>(outArray.data());
            uint32_t offset = sizeof(vk::ShaderSetHeader);
            for (int ix = 0; ix < vk::kShaderStageCount; ++ix) {
                ...
                if (i == vk::kShaderStageVertex && vsArray.size() > 0) {
                    ...
                    pOutHeader->shaders[i].offsetBytes = offset;
                    memcpy(outArray.data() + offset, datArray.data() + pHeader->shaders[i].offsetBytes, pHeader->shaders[i].size);
                    offset += pOutHeader->shaders[i].size;
                }
            }
